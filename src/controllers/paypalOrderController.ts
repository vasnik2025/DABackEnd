...
type PayPalClient = paypal.core.PayPalHttpClient;
...
const DEFAULT_CANCEL_URL =
  resolveEnv('PAYPAL_CANCEL_URL', ['PAYPAL_CANCEL_RETURN_URL']) ||
  `${NORMALIZED_FRONTEND_BASE}/payments/paypal/cancel`;

let cachedClient: PayPalClient | null = null;
...
const resolveEnvironment = (): paypal.core.PayPalEnvironment => {
  const Environment =
    MODE === 'live'
...
const getClient = (): PayPalClient => {
  ...
  if (!cachedClient) {
    cachedClient = new paypal.core.PayPalHttpClient(resolveEnvironment());
  }
  return cachedClient;
};

const asOperationalError = (
  error: unknown,
  ...
  const status =
    Number.isFinite(statusCandidate) && statusCandidate >= 400 && statusCandidate < 600
      ? statusCandidate
      : fallbackStatus;

  const message =
    typeof (error as { message?: string }).message === 'string' &&
    (error as { message: string }).message.trim().length
      ? (error as { message: string }).message
      : fallbackMessage;

  const opError = new OperationalError(message, status);
  (opError as any).details = error;
  return opError;
};
...
export const createOrder = async (
  ...
  let requestPlanType: string | null = null;
  try {
    ...
    if (!userId) {
      return next(
        new OperationalError('userId is required to create an order.', 400),
      );
    }
    ...
    const links = Array.isArray(response?.result?.links)
      ? (response.result.links as Array<{ href?: string; rel?: string }>)
      : [];
    const approvalUrl =
      links.find((link) => link?.rel === 'approve' && typeof link?.href === 'string')
        ?.href ?? null;
    ...
    const paypalErrorCode = extractPaypalErrorCode(error);
    const paypalStatus = extractPaypalStatus(error);
    const message = extractErrorMessage(error, 'Failed to initialize PayPal payment');
    void safeRecordPaypalFailure({
      ...
      paypalErrorCode,
      paypalStatus,
      isConnectivity: isConnectivityError(error),
    ...
    next(
      asOperationalError(
        error,
        'Failed to initialize PayPal payment. Please try again later.',
      ),
    );
  }
};
...
export const captureOrder = async (
  ...
  try {
    ...
    if (!orderId) {
      return next(
        new OperationalError('orderId is required to capture an order.', 400),
      );
    }
    ...
    const paypalErrorCode = extractPaypalErrorCode(error);
    const paypalStatus = extractPaypalStatus(error);
    const message = extractErrorMessage(error, 'Failed to capture PayPal payment');
    void safeRecordPaypalFailure({
    ...
    next(
      asOperationalError(
        error,
        'Failed to capture PayPal payment. Please try again later.',
      ),
    );
  }
};
