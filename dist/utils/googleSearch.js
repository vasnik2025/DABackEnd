...
exports.searchLifestyleWithGoogle = void 0;
...
const formatDescription = (place, formattedTypes) => {
    const highlights = [];
    if (typeof place.rating === "number") {
        const ratingNumber = place.rating.toFixed(1);
        const ratingText = place.user_ratings_total && place.user_ratings_total > 0
    ...
    if (typeof place.business_status === "string" &&
        place.business_status.length &&
        place.business_status !== "OPERATIONAL") {
        highlights.push(toTitleCase(place.business_status.replace(/_/g, " ").toLowerCase()));
    }
...
const buildBaseQuery = (searchType, country, city, _vibe, countryCode) => {
    ...
    const locationLabel = cleanedCity && cleanedCountry
...
const searchLifestyleWithGoogle = async (searchType, country, city, vibe, countryCode) => {
    ...
    const { query, locationLabel, regionCode } = buildBaseQuery(searchType, country, city, vibe, countryCode);
    ...
    if (payload.status === "REQUEST_DENIED") {
        const denialMessage = typeof payload.error_message === "string" ? payload.error_message : "No reason provided";
        console.warn(`[googleSearch] Places API request denied: ${denialMessage}. Falling back to generic Google search.`);
        return {
            ...emptyResponse,
    ...
    if (payload.status !== "OK") {
        throw new Error(`Google Places error: ${payload.status}${payload.error_message ? ` - ${payload.error_message}` : ""}`);
    }
    ...
    const enriched = await Promise.all(places.map(async (place) => {
        ...
        return {
            ...
            reviewCount: typeof place.user_ratings_total === "number"
    ...
    const results = enriched.filter((entry) => Boolean(entry?.name));
    return {
        results,
        query,
        searchUrl: `https://www.google.com/search?q=${encodeURIComponent(query)}`,
...
exports.searchLifestyleWithGoogle = searchLifestyleWithGoogle;
