...
exports.handleGetZodiacPrediction = handleGetZodiacPrediction;
const ZODIAC_TRAITS = {
...
const formatZodiacLabel = (sign) => {
    if (!sign)
        return 'Your Sign';
    return sign.charAt(0).toUpperCase() + sign.slice(1).toLowerCase();
};
const pickFromList = (list, seed) => list[Math.abs(seed) % list.length];
const buildLocalZodiacPrediction = (rawSign) => {
...
async function handleGetZodiacPrediction(req, res, next) {
    try {
